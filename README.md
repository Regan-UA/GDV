# GDV

## M2
### les 1

Hier heb ik een systeem voor het genereren van deeltjes.

![les1 preview before](M2/les1/before.gif)

Het genereert alleen zware ionen.
Maar ik kan elektronen toevoegen aan het scriptbare objectrecept in de inspecteur (het gebruikt lijsten en hun indexen om deeltjes te identificeren).

![screnshot](M2/les1/Screenshot.png)

Hier is het resultaat.

![les1 preview after](M2/les1/after.gif)

### School project 1
Project is in Releases - project 1.

Dit is een sort van biljard spel.

![gameplay preview](M2/project1/gameplay.gif)

In dit spel je krijgt meer score met elke bal je of andere geactiveerd bal raakt. Key feature is dat met elke bal van hetzelfde kleur je activeerd je krijgt score multiplier +0.5. 

Hier is code.

![Player controller](M2/project1/playerController1.png)

In Update method is heel simpel controller. Alle andere methoden zijn gewoon berekeningen.

![Player controller](M2/project1/playerController2.png)

Dan code voor elke gekleurde bal op het scene:

![scoreBall](M2/project1/scoreBall.png)

Score berekeningen gaan door game manager:

![Game manager](M2/project1/gameManager.png)